...
def supervisor_getSelectteacher(limit, offset,search):
    ...
    try:
        ...
        sql = """SELECT a.id,name,user_id,sex,tel,institution_name FROM app_select_sysuser a
...
def supervisor_getSelectstudent( limit, offset,search):
...
def supervisor_setDate(
    ...
    try:
        ...
        obj.select3_end = tea_select_end
...
def director_getSelectstudent(institution_id,tea_no,class_id,stustu,limit,offset):
    ...
    try:
        ...
        sql = '''SELECT COUNT(*) FROM app_select_sysuser a
                ...
                LEFT JOIN app_sex e ON e.id=c.sex_id
                LEFT JOIN app_class f ON c.stu_class = f.class_name
                WHERE  a.institution_id=''' + institution + " and b.year=" + "'" + year[
            'year'] + "'" + " and a.achieve_year=" + "'" + year['year'] + "'" + " and c.achieve_year=" + "'" + year[
                  'year'] + "'"
        if tea_no != '' and tea_no != '-1':
            sql = sql + ' AND a.id=' + str(tea_no)
        if class_id != '' and class_id != '-1':
...
def getTeaToopic(limit, offset, tea_no,userid):
    ...
    try:
        ...
        sql = '''SELECT COUNT(*) FROM app_topic a  LEFT JOIN app_select_sysuser c ON c.user_id=a.tea_no
                 LEFT JOIN  app_sex d ON c.sex_id=d.id
                 LEFT JOIN  app_date_setting e ON a.year=e.year
                 WHERE e.activation=1 '''
        sql = sql + ' AND c.institution_id=' + str(institutionid)
...
def director_uptopic(filename):
    try:
        ...
        for i in range(3, max_row+1):
            ...
            subject = L[2]
            tea_no = L[1]
            major = Major.objects.get(major=L[14])
            major_id = major.id
            introduction = L[3]
            if L[4]:
...
def getTeaToopicnum(limit, offset, institutionid):
    try:
        ...
        sql = """SELECT a.id,a.name,a.user_id,b.topic_num FROM app_select_sysuser a
...
def director_Numberbyid(eid):
    ...
    try:
        cursor = connection.cursor()
        sql = """SELECT a.id,a.name,a.user_id,b.topic_num FROM app_select_sysuser a
                      LEFT JOIN app_teacher_topic_num b ON b.tea_no="""+str(teano)
...
def getTeaTopic(limit, offset, userid):
    achieveyear = Date_setting.objects.get(activation=1)
    achieve_year = achieveyear.year
    try:
        cursor = connection.cursor()
        sql = '''SELECT  a.id,a.subject,a.introduction,a.year,b.selected_stu_no,b.submit,c.name,c.stu_class,c.user_id ,d.sex,e.year,e.activation,f.major FROM app_topic a
                 LEFT JOIN app_application_state b ON b.topic=a.id
                 LEFT JOIN app_select_sysuser c ON c.user_id=b.selected_stu_no
                 LEFT JOIN  app_sex d ON c.sex_id=d.id
                 LEFT JOIN  app_date_setting e ON a.year=e.year
                 LEFT JOIN app_major f ON f.id=a.major_id
                 WHERE e.activation=1 AND a.tea_no='''
        sql = sql + str(userid) + ' LIMIT ' + str(limit) + ' OFFSET ' + str(offset)
        print sql
        cursor.execute(sql)
        select_out = dictfetchall(cursor)

        sql = '''SELECT COUNT(*) FROM app_topic WHERE tea_no=''' +  str(userid)+" and year="+achieve_year
        cursor.execute(sql)
        num = dictfetchall(cursor)[0]
        data = {'total': num['COUNT(*)'], 'rows': select_out}
        return data
...
def getTeaTopic1(limit, offset, userid):
    achieveyear = Date_setting.objects.get(activation=1)
    achieve_year = achieveyear.year
    try:
        cursor = connection.cursor()
        ...
        sql = '''SELECT COUNT(*) FROM app_topic1 WHERE tea_no=''' +  str(userid)+" and year="+achieve_year
        cursor.execute(sql)
        num = dictfetchall(cursor)[0]
        data = {'total': num['COUNT(*)'], 'rows': select_out}
        return data
...
def supervisor_setTopicnum(
    ...
    try:
        ...
        thisyearinstitution = institution.filter(year=achieve_year)
        if len(thisyearinstitution) > 0:
            return 0, '该研究所已设置'
        else:
...
def director_Get_minmax(institution_id):
    ...
    try:
        ...
        achieve_year = achieveyear.year
        cursor = connection.cursor()
        sql = '''SELECT  a.max,a.min FROM app_institution_topic_num a
                 WHERE a.year='''+ str(achieve_year)+" AND a.Institution_id="+str(institution_id)
        print sql
        cursor.execute(sql)
        return 1, dictfetchall(cursor)
    except:
